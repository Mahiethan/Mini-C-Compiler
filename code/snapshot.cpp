...
// AllocaInst *VariableASTnode::codegen() {
  ...
  if(NamedValues.insert({Val,varAlloca}).second == false) //check if symbol table already contains same variable name
  {
    ...
    errs()<<"Semantic error: Redefinition of variable "<<Val<<" with different type "<<Type<<". Variable "<<Val<<" of type "<<existTy<<" already exists.\n";
    return nullptr;
  }
...
Value* UnaryExprASTnode::codegen(){
  ...
  if(Opcode == "!")
    return Builder.CreateNot(operand,"nottemp");
  else if(Opcode == "-")
    return Builder.CreateNeg(operand,"negtemp");
  else
    return nullptr;
  // return nullptr;
}
...
Value* BinaryExprASTnode::codegen(){
  ...
  // }
    ...
    if(Opcode == "=") //ASSIGN
    {
      ...
      {
        cout<<"name:"<<name<<endl;
        AllocaInst *storePtr = NamedValues[name];
        if(!storePtr)
        {
          errs()<<"Semantic error: Unknown variable name: "<<name<<"\n";
          return nullptr;
        ...
        else if(lhsType > rhsType)
        {
          ...
          return Builder.CreateStore(rhs,l);
      }
      else
        return nullptr;
    }
      ...
      if(Opcode == "||") //OR
      {
        //need to convert both operands to bool type (if not already)
          ...
          if(rhsType == 2)
          {
            ...
            lhs = Builder.CreateIntCast(lhs, Type::getInt1Ty(TheContext), false, "bool_cast");
            rhs = Builder.CreateIntCast(rhs, Type::getInt1Ty(TheContext), false, "bool_cast");
      ...
      {
        return Builder.CreateFCmpOEQ(lhs,rhs,"eq_tmp");
      }
      else if(Opcode == "!=") //NEQ
        return Builder.CreateFCmpONE(lhs,rhs,"neq_tmp");
      else if(Opcode == "<=") //LE
        return Builder.CreateFCmpOLE(lhs,rhs,"leq_tmp");
      else if(Opcode == "<") //LT
        return Builder.CreateFCmpOLT(lhs,rhs,"lt_tmp");
      else if(Opcode == ">=") //GE
        return Builder.CreateFCmpOGE(lhs,rhs,"geq_tmp");
      else if(Opcode == ">") //GT
        return Builder.CreateFCmpOGT(lhs,rhs,"gt_tmp");
      else if(Opcode == "+") //PLUS
      {
...
Value* ReturnExprASTnode::codegen(){
  ...
  if(returnExpr != nullptr)
  {
    ...
    {
      if(AI->getAllocatedType()->isFloatTy())
        returnExpr = Builder.CreateLoad(Type::getFloatTy(TheContext), AI);
      else if(AI->getAllocatedType()->isIntegerTy(32))
        returnExpr = Builder.CreateLoad(Type::getInt32Ty(TheContext), AI);
      else if(AI->getAllocatedType()->isIntegerTy(1))
        returnExpr = Builder.CreateLoad(Type::getInt1Ty(TheContext), AI);
    }
